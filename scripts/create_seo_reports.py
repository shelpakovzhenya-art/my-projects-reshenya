...
REPORTS = [
    ...
    },
    {
        ...
        "done": [
            "Вход в ICMS работает, SEO-раздел проверен.",
            ...
            "/prays_list закрыт: страница была в статусе Редактируется, переведена в Опубликован и теперь отдает 200.",
            "canonical не выводится. В редакторе страниц, параметрах и метриках CMS поля canonical/head нет.",
            "Яндекс.Вебмастер не открыт: нужен Яндекс ID, рабочего доступа в таблице нет.",
        ],
        "tz": [
            ("P1", "Исправить серверный редирект со слэшем", "Сначала сохранить .htaccess/серверные правила. Убрать правило, которое отправляет URL со слэшем на главную. Настроить 301: /uslugi/ -> /uslugi, /kontakty/ -> /kontakty, /stati/ -> /stati, /galereya/ -> /galereya, /prays_list/ -> /prays_list."),
        ...
        "facts": [
            ...
            "/prays_list без слэша теперь отдает 200 после публикации страницы в CMS.",
